...
with tab1:
    ...
    with kpi1:
        st.metric("Total Net Revenue", f"₹{net_rev:,.0f}")
    with kpi2:
    ...
    with c1:
        ...
        if 'Branch Name' in df.columns and 'Net Amount' in df.columns:
            branch_df = df.groupby('Branch Name')['Net Amount'].sum().reset_index().sort_values(by='Net Amount', ascending=True)
...
with tab2:
    ...
    # Horizontal filters for better UX
...
with tab3:
    ...
    if not matrix_df.empty:
        pivot_matrix = matrix_df.pivot_table(
            ...
            columns='Branch Name',
            values='Net Amount',  # Focusing on Net Amount for Heatmap clarity
            aggfunc='sum',
            fill_value=0
        )
        
        # Apply heatmap styling to highlight highest revenue numbers
